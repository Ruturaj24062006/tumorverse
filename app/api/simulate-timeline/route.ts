...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as TimelineRequestBody
    ...
    const response = await fetch(`${API_BASE_URL}/simulate-timeline`, {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data?.detail || data?.error || "Failed to simulate timeline" }, { status: response.status })
    }

    return NextResponse.json(data)
  } catch (error) {
    console.error("simulate-timeline route error", error)
    return NextResponse.json({ error: "Failed to generate tumor timeline animation" }, { status: 500 })
  }
}
